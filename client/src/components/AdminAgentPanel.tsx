...
export function AdminAgentPanel() {
  ...
  const submitMessage = async (event?: FormEvent) => {
    ...
    try {
        ...
        onEvent: ({ event, data }) => {
          if (event === "agent.token") {
            ...
            updateActiveSession((session) => {
              ...
              const lastIndex = messagesCopy.map((item) => item.role).lastIndexOf("assistant");
              if (lastIndex < 0) return session;
              const currentContent = messagesCopy[lastIndex].content;
      ...
      updateLastAssistant({ content: "Agent request failed before a final answer was returned." });
    } finally {
  ...
  return (
    <div className="grid min-h-[calc(100vh-132px)] w-full grid-cols-[300px_minmax(0,1fr)] bg-slate-50 dark:bg-slate-900">
      <aside className="bg-white dark:bg-slate-950 p-4">
      ...
      <section className="flex min-w-0 flex-col">
        <div className="bg-white dark:bg-slate-950 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-semibold text-slate-950 dark:text-white">Telesales Agent</h3>
          </div>
        </div>
          ...
          {messages.length === 0 ? (
            <div className="flex h-full min-h-[420px] items-center justify-center">
              <div className="max-w-lg text-center">
                ...
                <h3 className="text-xl font-semibold text-slate-950 dark:text-white">Ask a telesales question</h3>
                <p className="mt-2 text-sm text-slate-500">
                  The agent will search products, inspect offers, compare prices, and return a sales-ready answer with sources.
                </p>
              </div>
            </div>
          ) : (
                    ...
                    {message.recommendations && message.recommendations.length > 0 && (
                      <div className="mt-4 space-y-2">
                        {message.recommendations.slice(0, 3).map((recommendation) => (
                          <div key={recommendation.product_id} className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900">
                            <p className="text-xs font-semibold text-slate-950 dark:text-white">{recommendation.product_name}</p>
                            <p className="mt-1 text-[11px] text-slate-500">{formatPrice(recommendation.lowest_price)}</p>
