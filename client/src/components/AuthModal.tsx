          ...
          <div className="p-8">
            ...
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-xl font-black tracking-tighter text-slate-950 dark:text-white font-display uppercase">
                  ProductHunter
                </span>
              </div>
              <button
            ...
            {/* Title */}
            <div className="mb-8">
              <h2 className="text-2xl font-black text-slate-950 dark:text-white font-display leading-tight tracking-tight uppercase">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {!isLogin && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    <User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 group-focus-within:text-brand-primary transition-colors pointer-events-none" />
                    <input
                      type="text"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 group-focus-within:text-brand-primary transition-colors pointer-events-none" />
                  <input
                    type="email"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative group">
                  <Lock className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 group-focus-within:text-brand-primary transition-colors pointer-events-none" />
                  <input
                    type="password"
                    placeholder="••••••••"
                    value={password}
              ...
              <div className="pt-1">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="group flex w-full items-center justify-center gap-2.5 rounded-xl bg-brand-primary py-4 text-xs font-black text-white shadow-xl shadow-brand-primary/20 transition-all hover:bg-brand-secondary active:scale-95 uppercase tracking-widest font-display disabled:opacity-70 disabled:pointer-events-none"
                >
                  {isLoading ? (
            ...
            <div className="relative my-7">
              ...
              <div className="relative flex justify-center">
                <span className="bg-white dark:bg-slate-900 px-4 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">
                  {t('orContinueWith')}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                onClick={() => window.location.href = `${CONFIG.API_URL}/auth/google/login?frontend_url=${encodeURIComponent(window.location.origin)}`}
                className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-3 text-xs font-bold text-slate-700 dark:text-slate-300 transition-all hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95 shadow-sm"
              >
              ...
              <button
                type="button"
                onClick={() => window.location.href = `${CONFIG.API_URL}/auth/github/login?frontend_url=${encodeURIComponent(window.location.origin)}`}
                className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-3 text-xs font-bold text-slate-700 dark:text-slate-300 transition-all hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-95 shadow-sm"
              >
