...
export function ProductCard({
  ...
  const normalizeImageUrl = (url: string | null | undefined): string | null => {
    ...
    if (cleaned.startsWith("//")) {
      ...
      cleaned.startsWith("cdn2.fptshop.com.vn") ||
      cleaned.startsWith("fptshop.com.vn")
    ) {
      cleaned = `https://${cleaned}`;
    }
    return cleaned || null;
  ...
  const productImage = isRealImageUrl(normalizedImageUrl)
    ? normalizedImageUrl
    : null;
  ...
  const displayPrice = isGroup
    ? (product.lowest_price ?? product.current_price ?? 0)
    : currentPrice;
  const displayOriginal = isGroup
    ? (product.original_price ?? product.lowest_price ?? 0)
    : originalPrice;
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="mb-4 text-sm font-medium text-slate-500 dark:text-slate-400">
                Theo dõi <strong>{product.raw_name}</strong>. Chúng tôi sẽ gửi
                email ngay khi giá giảm xuống mức này.
              </p>
